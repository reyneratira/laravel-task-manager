...
@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-gray-800">Edit Tugas</h1>
        <p class="text-sm text-gray-500 mt-1">Perbarui informasi tugas di bawah.</p>
    </div>

    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <form method="POST" action="{{ route('admin.tasks.update', $task) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500">{{ old('description', $task->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center gap-3 pt-2">
                ...
                <a href="{{ route('admin.tasks.show', $task) }}" class="text-sm text-gray-500 hover:text-gray-700">Batal</a>
            </div>
        </form>
    </div>
@endsection
